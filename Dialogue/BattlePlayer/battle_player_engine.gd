...
func _setup() -> void:
	...
	var entry: DialogueEntry = add_text_entry("回答正确，怪物看我的厉害!")
	entry.set_metadata_data({
		Game.AUTHOR:"少侠",
		Game.AUDIO:preload("res://Assets/Audio/Dialogue/BattlePlayer01.wav"),
	})
	
	#var enemy_entry : DialogueEntry = add_text_entry("回答正错误，小子受死吧!", ENEMY)
